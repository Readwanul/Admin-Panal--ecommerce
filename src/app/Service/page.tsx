...
// Define the TypeScript interface for HouseOwner data
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3001/service-provider/all")
      .then((res) => res.json())
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-purple-700 text-white p-5">
        ...
        <button className="mt-10 bg-red-500 px-4 py-2 rounded w-full">Logout</button>
      </aside>

      {/* Main Content */}
      <main className="flex-1 p-6">
        <h1 className="text-2xl font-semibold mb-4">Service Provider</h1>
        <div className="mb-4">
              ...
              {houseOwners
                ...
                .map((owner) => (
                  <tr key={owner.id} className="hover:bg-gray-100">
                    ...
                    <td className="border border-gray-400 px-4 py-2">{owner.Service}</td>
                    <td className="border border-gray-400 px-4 py-2">{owner.Gender}</td>
                    <td className="border border-gray-400 px-4 py-2 text-center">
                      <button
                        className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600"
                        onClick={() => router.push(`Service/profile/${owner.id}`)}
                      >
                        View Profile
                      </button>
                    </td>
